...
export default function SubscribeForm({ compact = false }: { compact?: boolean }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.error) {
      setStatus('error');
      setErrorMsg(t('al.subscribeError').replace('{err}', String(result.error)) + ' alert_subscribers → Supabase SQL editor.');
    } else {
      setStatus('done');
      setEmail('');
      setRegion('');
    ...
    return (
      <div>
        <div className="flex items-center gap-1.5 mb-1.5">
          ...
          <span className="text-[10px] text-slate-400 uppercase tracking-wider">{t('al.emailNotify')}</span>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-1.5">
          <input
            type="email"
            required
            placeholder={t('al.emailPlaceholder')}
            value={email}
          ...
          <input
            type="text"
            placeholder={t('al.regionPlaceholder')}
            value={region}
          ...
          >
            <Send size={11} />
            {status === 'sending' ? t('al.subscribing') : t('al.subscribeBtn')}
          </button>
        </form>
      </div>
    );
  ...
  return (
    <div className="bg-satellite-card border border-satellite-border rounded-xl p-4">
      <div className="flex items-center gap-2 mb-1">
        ...
        <span className="text-sm font-semibold text-slate-200">{t('al.emailNotify')}</span>
      </div>
      <p className="text-[11px] text-slate-500 mb-3">
        {t('al.emailNotifyDesc')}
      </p>
