...
const FEATURES = [
  {
    ...
    desc: 'Convert grayscale thermal IR images to vivid scientific colormaps (JET, TURBO, INFERNO, PLASMA) with adjustable intensity.',
    page: 'colorize',
    color: '#FF6B35',
  ...
  },
  {
    ...
    desc: 'Compute PSNR, SSIM, FID metrics and cloud coverage percentage from the uploaded satellite imagery.',
    page: 'colorize',
    color: '#10B981',
  ...
  },
  {
    ...
    desc: 'AI-powered detection of weather conditions, vegetation (NDVI), water bodies, and urban heat islands.',
    page: 'colorize',
    color: '#3B82F6',
  ...
  },
  {
    ...
    desc: 'Compare before & after satellite images to detect affected areas, assess damage severity, and trigger alerts.',
    page: 'change',
    color: '#EF4444',
  ...
  },
  {
    ...
    desc: 'Switch between INSAT-3DS (state), Sentinel-1 (district), and Cartosat-3 (street) level satellite data.',
    page: 'colorize',
    color: '#F59E0B',
  ...
  },
  {
    ...
    desc: 'Automatic anomaly detection with alert generation when cloud coverage, heat, or change thresholds are exceeded.',
    page: 'alerts',
    color: '#8B5CF6',
  ...
  return (
    <div className="space-y-8 pb-8">
          ...
          <div className="flex items-start justify-between flex-wrap gap-6">
            <div className="max-w-2xl">
              ...
              <h1 className="text-3xl md:text-4xl font-bold text-white leading-tight">
                <span className="text-accent-orange">IR Image</span> Colorization<br />
                & Disaster Analysis
              </h1>
              <p className="mt-3 text-slate-400 text-sm leading-relaxed max-w-lg">
                Transform raw infrared satellite imagery from INSAT, Landsat, and Cartosat into
                actionable insights for weather analysis, vegetation mapping, and disaster management.
              </p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: 'Analyses Performed', value: summary.totalAnalyses, icon: Activity, color: '#FF6B35' },
          { label: 'Avg PSNR Quality', value: summary.avgPsnr > 0 ? `${summary.avgPsnr.toFixed(1)} dB` : '—', icon: Satellite, color: '#10B981' },
          { label: 'Avg Cloud Coverage', value: summary.avgCloudCoverage > 0 ? `${summary.avgCloudCoverage.toFixed(0)}%` : '—', icon: Globe, color: '#3B82F6' },
          { label: 'High Severity Events', value: summary.highSeverityCount, icon: Shield, color: '#EF4444' },
        ].map(stat => (
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-semibold text-slate-200 uppercase tracking-wider">Platform Features</h2>
          <span className="text-[10px] text-slate-500 font-mono">6 modules active</span>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {FEATURES.map(f => (
            ...
            >
              ...
              <div className="text-sm font-semibold text-slate-200 mb-1.5">{f.title}</div>
              <div className="text-xs text-slate-500 leading-relaxed mb-3">{f.desc}</div>
      ...
      <div className="bg-satellite-card border border-satellite-border rounded-xl p-6">
        <div className="text-xs text-slate-400 uppercase tracking-wider mb-5">End-to-End Workflow</div>
        <div className="flex flex-wrap items-center gap-2">
          {[
            { step: '01', label: 'Upload IR Image', color: '#FF6B35' },
            { step: '02', label: 'Preprocessing', color: '#F59E0B' },
            { step: '03', label: 'Colorization Engine', color: '#10B981' },
            { step: '04', label: 'Quality Metrics', color: '#3B82F6' },
            { step: '05', label: 'Scene Analysis', color: '#8B5CF6' },
            { step: '06', label: 'Alerts & Export', color: '#EF4444' },
          ].map((s, i, arr) => (
